...
source "amazon-ebs" "windows" {
  ami_name      = "sbom-amzn-windows-no-exec"
...
hcp_packer_registry {
  ...
  description = "Amazon Windows SBOM test without execute_command override."
}

build {
  name    = "sbom-amazon-windows-no-exec"
  sources = ["source.amazon-ebs.windows"]

  provisioner "powershell" {
    inline = ["Write-Output 'packer works on amazon windows'"]
  }
  ...
  provisioner "hcp-sbom" {
    ...
    sbom_name     = "amzn-windows-no-exec"
