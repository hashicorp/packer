...
func (c *Client) WaitForCompletionRefRef(taskResponse map[string]interface{}) (waitExitStatus string, err error) {
...
func (c *Client) StatusChangeVm(vmr *VmRef, setStatus string) (exitStatus string, err error) {
	...
	for i := 0; i < 3; i++ {
		...
		exitStatus, err = c.WaitForCompletionRefRef(taskResponse)
		if exitStatus == "" {
			time.Sleep(TaskStatusCheckInterval * time.Second)
		} else {
			return
