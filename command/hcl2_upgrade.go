...
func (c *HCL2UpgradeCommand) writeAmazonAmiDatasource(builders []*template.Builder, out *bytes.Buffer) error {
		...
		for _, builder := range builders {
			if strings.HasPrefix(builder.Type, "amazon-") {
				if sourceAmiFilter, ok := builder.Config["source_ami_filter"]; ok {
					...
					unique := true
					for _, filter := range amazonAmiFilters {
	...
	if len(amazonAmiFilters) > 0 {
		out.Write([]byte(amazonAmiDataHeader))

		for i, filter := range amazonAmiFilters {
			datasourceContent := hclwrite.NewEmptyFile()
			body := datasourceContent.Body()
			body.AppendNewline()
			sourceBody := body.AppendNewBlock("data", []string{"amazon-ami", fmt.Sprintf("autogenerated_%d", i+1)}).Body()
			jsonBodyToHCL2Body(sourceBody, filter)
...
func jsonBodyToHCL2Body(out *hclwrite.Body, kvs map[string]interface{}) {
	...
	for _, k := range ks {
		...
		case map[string]string:
			if val, err := gocty.ToCtyValue(value, cty.Map(cty.String)); err == nil {
				out.SetAttributeValue(k, val)
			}
		case map[string]int, map[string]float64:
			out.SetAttributeValue(k, hcl2shim.HCL2ValueFromConfigValue(value))
		case []interface{}:
			if len(value) == 0 {
				continue
