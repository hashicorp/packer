...
func (ba *BuildArgs) AddFlagSets(flags *flag.FlagSet) {
	...
	flags.BoolVar(&ba.ReleaseOnly, "release-only", false, "force Packer to only load plugin releases")
...
func (va *ValidateArgs) AddFlagSets(flags *flag.FlagSet) {
	...
	flags.BoolVar(&va.ReleaseOnly, "release-only", false, "force Packer to only load plugin releases")
