...
func (b *CoreBuild) getPluginsMetadata() map[string]PluginDetails {
	...
	builderPlugin, builderPluginOk := GlobalPluginDetailStore.Get(PluginComponentBuilder, b.BuilderType)
	if builderPluginOk {
	...
	for _, pp := range b.PostProcessors {
		for _, p := range pp {
			postprocessorsPlugin, postprocessorsPluginOk := GlobalPluginDetailStore.Get(PluginComponentPostProcessor, p.PType)
	...
	for _, pv := range b.Provisioners {
		provisionerPlugin, provisionerPluginOk := GlobalPluginDetailStore.Get(PluginComponentProvisioner, pv.PType)
