...
func (s *StepTypeBootCommand) Run(state multistep.StateBag) multistep.StepAction {
	...
	// Determine the host IP from VNC connection
	vncLocalAddr := nc.LocalAddr()
	hostIP := strings.Split(fmt.Sprintf("%s", vncLocalAddr), ":")[0]

	log.Printf("Host IP for the Packer machine: %s", hostIP)
