...
func TestNewConfig_NetworkAdapter(t *testing.T) {
	...
	if config.NetworkAdapter != "e1000" {
...
func TestNewConfig_NetworkName(t *testing.T) {
	...
	if config.NetworkName != "VM Network" {
...
func TestNewConfig_GuestOSType(t *testing.T) {
	c := testConfig(t)

	delete(c, "guest_os_type")
	config, warns, errs := NewConfig(c)
	testConfigOk(t, warns, errs)

	if config.GuestOSType != "otherGuest" {
		t.Fatalf("bad guest os type: %s", config.GuestOSType)
	}

	c["guest_os_type"] = "rhel7-64"
	config, warns, errs = NewConfig(c)
	testConfigOk(t, warns, errs)

	if config.GuestOSType != "rhel7-64" {
		t.Fatalf("bad guest os type: %s", config.GuestOSType)
	}
}
...
func TestNewConfig_FloppyFiles(t *testing.T) {
	...
	delete(c, "floppy_files")
	config, warns, errs := NewConfig(c)
	testConfigOk(t, warns, errs)

	if len(config.FloppyFiles) != 0 {
		t.Fatalf("bad: %#v", config.FloppyFiles)
	}
	...
	if !reflect.DeepEqual(config.FloppyFiles, expected) {
		t.Fatalf("bad: %#v", config.FloppyFiles)
	}
}
...
func TestNewConfig_Format(t *testing.T) {
	...
	if config.Format != "ovf" {
		t.Fatalf("bad: %#v", config.Format)
	}
