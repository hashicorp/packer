...
func (s *StepCreateImage) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	imageCh, errCh := driver.CreateImage(
		config.ImageName, config.ImageDescription, config.ImageFamily, config.Zone, config.TargetDisk.Name,
		config.ImageLabels, config.ImageLicenses, config.ImageEncryptionKey.ComputeType(),
