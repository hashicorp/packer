...
func (c *Config) createInstanceMetadata(sourceImage *gcp.Image, sshPublicKey string) (map[string]string, error) {
	...
	if c.StartupScriptFile != "" {
		...
		instanceMetadata[gcp.StartupWrappedScriptKey] = string(content)
	} else if wrappedStartupScript, exists := instanceMetadata[gcp.StartupScriptKey]; exists {
		instanceMetadata[gcp.StartupWrappedScriptKey] = wrappedStartupScript
	}
	...
	if sourceImage.IsWindows() {
		...
		instanceMetadata[gcp.StartupScriptKey] = gcp.StartupScriptWindows
		instanceMetadata[gcp.StartupScriptStatusKey] = gcp.StartupScriptStatusDone
	} else {
		instanceMetadata[gcp.StartupScriptKey] = gcp.StartupScriptLinux
		instanceMetadata[gcp.StartupScriptStatusKey] = gcp.StartupScriptStatusNotDone
	}
...
func getImage(c *Config, d gcp.Driver) (*gcp.Image, error) {
...
func (s *StepCreateInstance) Run(ctx context.Context, state multistep.StateBag) multistep.StepAction {
	...
	disks := []*gcp.DiskConfig{
		{
			Name:   c.BootDisk.Name,
			SizeGb: c.BootDisk.SizeGb,
			Type:   c.BootDisk.Type,
			Boot:   true,
		},
		{
			Name:   c.TargetDisk.Name,
			SizeGb: c.TargetDisk.SizeGb,
			Type:   c.TargetDisk.Type,
			Boot:   false,
		},
	}

	errCh, err = d.RunInstance(&gcp.InstanceConfig{
...
func (s *StepCreateInstance) Cleanup(state multistep.StateBag) {
	...
	// Deleting the instance does not remove the boots. This cleanup removes
	// the disks.
