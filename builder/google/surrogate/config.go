...
//go:generate mapstructure-to-hcl2 -type Config,DiskConfig
...
type DiskConfig struct {
	...
	SizeGb int64  `mapstructure:"size" required:"false"`
...
type Config struct {
	...
	// The configuration of the disk from which to boot. If unset, a 10GB disk of
	// type pd-standard will be used, and given the same name as the instance.
	BootDisk *DiskConfig `mapstructure:"boot_disk" required:"false"`
	// The configuration of the target disk from which the image will be created.
	// If unset, a 10GB disk of type pd-standard will be used, and given a name
	// derived from the instance name.
	TargetDisk *DiskConfig `mapstructure:"target_disk" required:"false"`
	...
	//  }
	//  ```
	ImageEncryptionKey *gcp.CustomerEncryptionKey `mapstructure:"image_encryption_key" required:"false"`
...
func (c *Config) Prepare(raws ...interface{}) ([]string, error) {
	...
	// Boot disk
	if c.BootDisk == nil {
		c.BootDisk = &DiskConfig{}
	}

	if c.BootDisk.SizeGb == 0 {
		c.BootDisk.SizeGb = 20
	}

	if c.BootDisk.Type == "" {
		c.BootDisk.Type = "pd-standard"
	}

	// Target disk
	if c.TargetDisk == nil {
		c.TargetDisk = &DiskConfig{}
	}

	if c.TargetDisk.SizeGb == 0 {
		c.TargetDisk.SizeGb = 20
	}

	if c.TargetDisk.Type == "" {
		c.TargetDisk.Type = "pd-standard"
	}
	...
	if c.BootDisk.Name == "" {
		c.BootDisk.Name = fmt.Sprintf("%s-boot", c.InstanceName)
	}
	if c.TargetDisk.Name == "" {
		c.TargetDisk.Name = fmt.Sprintf("%s-target", c.InstanceName)
	}
	...
	if c.AccountFile != "" {
		...
		cfg, err := gcp.ProcessAccountFile(c.AccountFile)
		if err != nil {
			errs = packer.MultiErrorAppend(errs, err)
		}
		c.account = cfg
