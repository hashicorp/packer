...
func NewDriver(dconfig *DriverConfig, config *SSHConfig, vmName string) (Driver, error) {
	...
	err := driver.Verify()
	log.Printf("Testing vmware driver %T. Success: %t", driver, err == nil)

	// If we suceeded, then return the driver.
	if err == nil {
		return driver, nil
	}

	// We didn't succeed, so return an friendly error message showing what failed
	errs := fmt.Sprintf("* %s\n", err.Error())
