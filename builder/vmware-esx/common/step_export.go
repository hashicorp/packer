...
func (s *StepExport) Run(_ context.Context, state multistep.StateBag) multistep.StepAction {
	...
	// If we're using ESX5 (yes we are), then we can export it.
	// So first we grab the path to ovftool...
	ovftool := GetOVFTool()
	if ovftool == "" {
	...
	// Notify the user that we're going to export the VM
	...
	// Finally use ovftool to export the virtual machine into the OutputDir
	...
	// Notify the user about the output from ovftool
