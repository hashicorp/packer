...
Post-processors run after each defined build. A post-processor takes the
`Artifact` from build/provision steps or the `Artifact` from a previous
`post-processor`. Post-processors are optional, and they can be used to upload
artifacts, re-package, or more. The list of available post-processors can be
found in the [post-processors](/docs/post-processors) section.
...
* The [post-processors block](/docs/from-1.5/blocks/build/post-processor) 
  allows to define one or more chain of `post-processor` that will take the
