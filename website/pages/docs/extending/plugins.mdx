...
`packer-TYPE-NAME`. For example, `packer-plugin-amazon` for a "multi-plugin"
binary named "amazon". This multi-plugin binary will make one or more plugins
available to use. Valid types for plugins are down this page more.
...
~> **Note**: Only _multi-plugin binaries_ are expected to work with Packer's plugin
manager. The legacy `builder`, `post-processor` and `provisioner` plugin types
will keep on being detected but Packer cannot install them automatically.
...
is named `my`, this would make the following parts available:
