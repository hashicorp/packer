<!-- Code generated from the comments of the Config struct in builder/google/surrogate/config.go; DO NOT EDIT MANUALLY -->
...
-   `boot_disk` (\*DiskConfig) - The configuration of the disk from which to boot. If unset, a 10GB disk of
    type pd-standard will be used, and given the same name as the instance.
    
-   `target_disk` (\*DiskConfig) - The configuration of the target disk from which the image will be created.
...
-   `image_encryption_key` (\*gcp.CustomerEncryptionKey) - Image encryption key to apply to the created image. Possible values:
