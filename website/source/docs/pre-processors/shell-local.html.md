---
description: |
    The shell-local Packer pre processor enables users to do some pre processing
    after artifacts have been built.
layout: docs
page_title: 'Local Shell - Pre-Processors'
sidebar_current: 'docs-pre-processors-shell-local'
---

# Local Shell Pre Processor

Type: `shell-local`

The local shell pre processor executes scripts locally during the pre
...
-   `use_linux_pathing` (bool) - This is only relevant to windows hosts. If you
    ...
    intend to use the shell-local pre-processor to run a bash script, please
...
The shell-local pre-processor was designed with the idea of allowing you to run
...
pre-processor, by modifying the `execute_command` and the `use_linux_pathing`
options in the pre-processor config.
...
pre-processor works to run it safely and easily. This understanding will save
...
If any pre-processor fails, the `packer build` stops and all interim artifacts
are cleaned up.
