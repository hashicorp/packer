...
-   `third_party_switch_compatibility` (boolean) - If true, will use a method
    compatible with third party distributed switches (like the Cisco 1000v) to
    get the virtual machine's IP address. This method requires that VMWare
    Tools is installed and running. This only has an effect if `remote_type` is
    enabled.
