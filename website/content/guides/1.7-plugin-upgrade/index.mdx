...
Second, the new server provides the Packer core with structured metadata that allows it to check the plugin's semantic versioning and use that in conjunction with `packer init` and the `required_plugins` block to make sure that users are using not only the correct plugin, but the correct _version_ of that plugin.
...
With the multi-component plugin example, it is important to note that the name of the binary must be in the form `packer-pluign-name`. Otherwise Packer will not register it as a multi-component plugin, nor download it.
...
The intention of multi-component plugins is to enable maintainers to serve related components together. A good naming convention is to name the plugin itself after your hypervisor or cloud environment, and the individual components after their specific functions.

Many Packer plugins already follow this naming convention. To use an example from the Packer core; if you were writing a custom VSphere plugin, you could now name the plugin `packer-plugin-vsphere`, and register different VSphere builders using:
...
And then reference your desired component in the Packer template using "vsphere-iso" and "vsphere-clone" just like Packer currently does.

You can still use the `DEFAULT_NAME` constant with a set implementation; in this example, whatever you register with the `DEFAULT_NAME` will be referenced in the Packer template as "vsphere". You can only register one component of each type (i.e. one builder, one provisioner, and one post-processor) per plugin using the `DEFAULT_NAME`.
...
If however, you would like to take advantage of Packer's automated plugin installation process via `packer init` -- the preferred method of installation -- you will need to make the plugin available on GitHub and in a repository named after the multi-component plugin.  `https://github.com/<yourorg>/packer-plugin-name`. We recognize that this may require you to rename or fork your plugin repository, but we think that it is worth the inconvenience to reduce ambiguity in the init call.
