...
and that fits in the requirements of a multi-component plugin. The easiest way to know if the plugin you use fits those
...
If the plugin you use is a multi-component plugin, you are ready to upgrade your template.

#### The plugin I need is not a multi-component plugin yet

With the SDK released at the same time as the `init` command, plugin users may face a gap between the release and the
plugin upgrade. Maintainers will have to upgrade their plugins and the process is a little bit more complicated than this one.
...
| Not Compatible | Manual and `packer init` Installation  | Manual and `packer init`  Installation | v1.7.6|
