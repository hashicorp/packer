...
The presence of a `hcp_packer_registry` block will enable HCP Packer mode and all the builds within that build block
...
the defined builders (source blocks) and the HCP Packer registry.
