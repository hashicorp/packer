...
If you want to exclude certain post-processors from your `packer build` call
...
_name_. The default post-processor name is its type, as demonstrated above. You
can make a post-processor's name unique by adding a "name" field to each
post-processor block.

While the command line "except" option can exclude post-processors, the command
line "only" option ignores post-processors. To use "only" to select
post-processors for particular builds, you must use the `only=[source]` syntax
inside of your hcl templates, as described above.
