...
when using the `templatefile`, these files *are* hcl files but also templates
and a slightly different set of features than the ones offered in a Packer
template, whilst still being specific to Packer.
