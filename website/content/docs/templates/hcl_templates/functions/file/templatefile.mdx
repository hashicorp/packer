...
-> *Recommendation:* we recommend on using the `.pkrtpl.hcl` extension file.
When using the `templatefile` functions, template files *are* treated as hcl
files but also templates and have slightly different set of features than the
ones offered in a Packer template. They are still specific to Packer.
