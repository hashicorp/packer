...
when using the `templatefile` functions, template files *are* hcl files but also
templates and have slightly different set of features than the ones offered in a
Packer template, whilst still being specific to Packer.
