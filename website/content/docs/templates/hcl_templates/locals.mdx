...
description: >-
  Local variables assign a name to an expression that can then be used multiple
...
definitions files. Local variables cannot be overridden.
...
Local variables are defined in `local` or `locals` blocks:
...
# Local variables can be interpolated elsewhere using the "local." prefix.
