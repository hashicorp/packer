...
This block is also very useful to define complex locals. Packer might take some time to expands and evaluate `locals`
with complex expressions dependent on other locals. The `locals` block is read as map and maps are not sorted,
and the evaluation time is not deterministic.

To avoid that, singular `local` block should be used instead. These will be
