...
1. Make any necesary registry edits to enable remote execution
   (and remote execution with elevated privileges, if needed)
1. Start WinRM, setting any config needed for allowing basic auth
1. Open ports 5985 and/or 5986 depending on how you're connecting
1. launch WinRM and set it to automatically launch when the computer restarts
1. If necessary, generate a self-signed certificate or provide a real certificate
