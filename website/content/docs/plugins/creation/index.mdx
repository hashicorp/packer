...
To install a dev version of a plugin based out of [packer-plugin-scaffolding](https://github.com/hashicorp/packer-plugin-scaffolding), you can use the following command:
```shell-session
  $ make dev
```
This command creates the dev binary of the plugin and also installs it for packer to use.

To install plugins from local source, you can also use the following command:
...
For example lets install happycloud plugin from a locally sourced binary
