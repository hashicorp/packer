...
1. Add a [`required_plugin`](http://localhost:3000/docs/templates/hcl_templates/blocks/packer#specifying-plugin-requirements)
block to your `packer` block, each block will tell packer what version to use,
make sure to set a correct [version constraint
string](http://localhost:3000/docs/templates/hcl_templates/blocks/packer#version-constraints). For example:
...
2. Run `packer init` to install all missing plugin binaries.

### Auto-resolver

In the previous examples, the getter will look for plugins located at:
* github.com/azr/packer-plugin-aws
* github.com/hashicorp/packer-plugin-azure

Plugins should always be addressed using this dns resolvable form. The plugin
getter defaults the domain to `github.com` and the account to `hashicorp` if
unspecified. The plugin name should always skip the `packer-plugin-` prefix that
will be automatically added.

Packer will install these plugins in your prefered folder, so here for example
on a `darwin_amd64` :
