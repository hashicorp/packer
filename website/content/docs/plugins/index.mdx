...
1. The `$HOME/.packer.d/plugins` directory, if `$HOME` is defined (unix)

1. The `%APPDATA%/packer.d/plugins` if `%APPDATA%` is defined (windows)

1. The `%USERPROFILE%/packer.d/plugins` if `%USERPROFILE%` is defined
   (windows)

1. The current working directory.

1. The directory defined in the env var `PACKER_PLUGIN_PATH`. There can be more
