...
-> **Note:** Downlading from *Github*, set the `PKR_GITHUB_API_TOKEN` env var in
order to get more requests per hour. Currently, the plugin getter only fetches
binaries using Github's public API, which [limits the number of requests per
hour one can
do](https://docs.github.com/en/developers/apps/rate-limits-for-github-apps#normal-user-to-server-rate-limits).
Packer will do its best to avoid hitting those limits. Go to your personal
[access token page](https://github.com/settings/tokens) to generate a new token.
