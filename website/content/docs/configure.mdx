...
the config directory will be: `/home/packer/.packer.d/` and other values will not
be checked.
* On a 'unix' system, if `HOME` is `/home/azr` or `USER` is `azr`, then
the config directory will default to `/home/azr/.packer.d/`.
* On a 'windows' system, if `PACKER_CONFIG_DIR` is set to `C:/`,the config
directory will be: `C:/packer.d/` and other values will not be checked.
