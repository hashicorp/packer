...
| unix                                  | windows                              |
|---------------------------------------|--------------------------------------|
| `${PACKER_CONFIG_DIR}/.packer.d/`     | `%PACKER_CONFIG_DIR%/packer.d/`      |
| `${APPDATA}/.packer.d/`               | `%APPDATA%/packer.d/`                |
| `${HOME}/.packer.d/`                  | `%HOME%/packer.d/`                   |
| user dir of `${USER}` + `/.packer.d/` | user dir of `${USER}` + `/packer.d/` |

Examples: 
* On a 'unix' system, if `PACKER_CONFIG_DIR` is set to `/home/packer`,
`PACKER_HOME_DIR` will be: `/home/packer/.packer.d/` and other values will not
be checked.
* On a 'unix' system, if `HOME` is `/home/azr` or `USER` is `azr`, then
`PACKER_HOME_DIR` will default to `/home/azr/.packer.d/`.
* On a 'windows' system, if `PACKER_CONFIG_DIR` is set to `C:/`,
`PACKER_HOME_DIR` will be: `C:/packer.d/` and other values will not be checked.
...
Packer can optionally read a JSON file to set some core settings. The config
file of packer will be in the `/.packer.d/` directory of the first one of the
following env values to be set :

| unix                                     | windows                                   |
|------------------------------------------|-------------------------------------------|
| `${PACKER_CONFIG_DIR}/.packerconfig`     | `%PACKER_CONFIG_DIR%/packer.config/`      |
| `${APPDATA}/.packerconfig`               | `%APPDATA%/packer.config/`                |
| `${HOME}/.packerconfig`                  | `%HOME%/packer.config/`                   |
| user dir of `${USER}` + `/.packerconfig` | user dir of `${USER}` + `/packer.config/` |
