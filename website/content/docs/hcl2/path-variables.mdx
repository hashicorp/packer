...
- [`abspath`](/docs/hcl2/functions/file/abspath) takes a string containing
  a filesystem path and converts it to an absolute path.

- [`basename`](/docs/hcl2/functions/file/basename) returns _only_ the last
  portion of a filesystem path, discarding the portion that would be returned
  by `dirname`.

- [`fileset`](/docs/hcl2/functions/file/fileset) enumerates a set of
  regular file names given a path and pattern.

- [`dirname`](/docs/hcl2/functions/file/dirname) returns all of the
