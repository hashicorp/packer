...
syntax](/docs/hcl2/syntax), which is designed to be easy for humans to read and
update.
...
- Where a property corresponds to an argument that accepts
  [arbitrary expressions](/docs/hcl2/expressions) in the native syntax, the
...
[expression syntax](/docs/hcl2/expressions), JSON values interpreted as expressions
...
| String  | Parsed as a [string template](/docs/hcl2/expressions#string-templates) and then evaluated as described below. |
...
expected, its value is first parsed as a [string template](/docs/hcl2/expressions#string-templates)
