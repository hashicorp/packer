...
- Maps/objects are represented by a pair of curly braces containing a series of
  ...
  they are a valid [identifier](/docs/hcl2/syntax#identifiers), but must be quoted
...
- `source.<SOURCE TYPE>.<NAME>` is an object representing a
  [source](/docs/hcl2/blocks/source) of the given type
  and name.

### Available Functions

For a full list of available functions, see [the function
reference](/docs/configuration/hcl2/functions).
...
[`flatten`](/docs/configuration/hcl2/functions/collection/flatten) and
[`setproduct`](/docs/configuration/hcl2/functions/collection/setproduct)
functions.
