---
description: |
  The Amazon AMI datasource provides information from an AMI that will be fetched based
  on the filter options provided in the configuration.

page_title: Amazon AMI - Datasources
sidebar_title: Amazon AMI
---

# Amazon AMI Datasource

Type: `amazon-ami`

The Amazon AMI datasource will filter and fetch an Amazon AMI, and output all the AMI information that will
be then available to use in the [Amazon builders](/docs/builders/amazon/instance).

-> **Note:** Datasources is a feature exclusively for HCL2 templates.
...
```
This selects the most recent Ubuntu 16.04 HVM EBS AMI from Canonical. Note that the datasource will fail unless
...
- `tags` - The key/value combination of a tags assigned to the AMI.
