---
description: |
  The `hcp_packer_version` data source retrieves information about
  an HCP Packer version from the HCP Packer registry. Use this information to
  get a source's external identifier.
page_title: hcp_packer_version data source reference
---
...
The `hcp_packer_version` dsata source retrieves information about
...
If an HCP Packer Version is revoked, the `hcp_packer_version` data source will fail and Packer won't proceed with
...
This data source can be used in conjunction with the `hcp_packer_artifact`
...
source "amazon-ebs" "packer-secondary" {
  source_ami       = data.hcp_packer_artifact.example.external_identifier
  ...
}
```
...
@include 'datasource/hcp_packer_version/Config-required.mdx'
...
@include 'datasource/hcp_packer_version/DatasourceOutput.mdx'
