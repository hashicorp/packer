* [`packer init`](/commands/init) will install plugins in the last directory in
the following list.
...
list. First plugin found takes precedence. Two binaries of the same plugin with
two different version will be considered as two different plugins. Highest
version matching `required_plugins` will be taken into consideration, if found.
...
* `PACKER_HOME_DIR/plugins/github.com/azr/aws/`
* `PACKER_HOME_DIR/plugins/github.com/hashicorp/azure/`

During initialization, on a `darwin_amd64` system, Packer will look-up for the
following files:

* `PACKER_EXEC_DIR/github.com/azr/aws/packer-plugin-aws_*_darwin_amd64_x5`
* `./github.com/azr/aws/packer-plugin-aws_*_darwin_amd64_x5`
* `PACKER_HOME_DIR/plugins/github.com/azr/aws/packer-plugin-aws_*_darwin_amd64_x5`
* `PACKER_PLUGIN_PATH/github.com/azr/aws/packer-plugin-aws_*_darwin_amd64_x5`
...
and refers to the directory wher `packer` is, or the executable directory.